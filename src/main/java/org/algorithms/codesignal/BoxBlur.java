...
public final class BoxBlur {
    ...
    public static final UnaryOperator<Integer[][]> SOLUTION = image -> {
        Integer[][] result = new Integer[image.length-2][image[0].length-2];
        for (int r = 0 ; r < image.length - 2 ; r++) {
            for (int c = 0 ; c < image[r].length - 2 ; c++) {
                int avg = (image[r][c] + image[r][c + 1] + image[r][c + 2] +
